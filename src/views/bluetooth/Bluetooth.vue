...
export default {
  ...
  methods: {
    resetSegmentsAndWaze(time) {
      ...
      if (this.mapLayerSelection.includes(3) && !this.mapLayerSelection.includes(4)) this.addMarkers(this.filteredWazeMarkers)
            ...
            deviceMarkers.forEach(m => {
              if (m.title == d.title) {
                if (device.status && m.zIndex != device.status.level) {
                  ...
                  m.setOptions({ zIndex: device.status.level, strokeColor: device.status.color, clickWindow: clickWindow})
                }
                return
              }
            })
    ...
    fetchHistorical(dt) {
      ...
      Api.fetchSegmentsHist(dt.ts).then(data => {
      ...
      Api.fetchSegmentsHistFull(dt.ts).then(data => {
        this.$store.state.bluetooth.apiData.segmentsFull = data
      }, error => {
        console.log(error)
      })
      Api.fetchWazeDataHist(60, dt.ts).then(data => {
      ...
      Api.fetchWazeDataHistFull(dt.ts).then(data => {
        this.$store.state.bluetooth.apiData.wazeFull = data
      }, error => {
        console.log(error)
      })
      Api.fetchDevicesHist(dt.ts).then(data => {
      ...
      Api.fetchDevicesHistFull(dt.ts).then(data => {
    ...
    fetchSegments(printInfo) {
      /* printInfo = true */
      Api.fetchSegments(printInfo).then(data => {
        this.$store.state.bluetooth.apiData.segments = data
        this.$store.state.bluetooth.apiData.routes = data.slice()
          .map(x => x.info.route)
      ...
      Api.fetchSegmentsFull(printInfo).then(data => {
        this.$store.state.bluetooth.apiData.segmentsFull = data
      }, error => {
        console.log(error)
      })
    ...
    fetchSensors(printInfo) {
      Api.fetchSensors(printInfo).then(data => {
    ...
    fetchDevices(printInfo) {
      Api.fetchDevicesBPData(printInfo).then(dataBP => {
        Api.fetchDevices(printInfo).then(dataDev => {
      ...
      Api.fetchDevicesFull(printInfo).then(data => {
        this.$store.state.bluetooth.apiData.devicesFull = data
      }, error => {
        console.log(error)
      })
    ...
    fetchWazeData(printInfo) {
      Api.fetchWazeData(printInfo).then(data => {
        this.$store.state.bluetooth.apiData.waze = data
        this.createWazeAlerts()
        if (this.mapLayerSelection.includes(3) && !this.mapLayerSelection.includes(4)) this.addMarkers(this.filteredWazeMarkers)
        if (this.mapLayerSelection.includes(3) && this.mapLayerSelection.includes(4)) this.addWazeClusters()
      ...
      Api.fetchWazeDataFull(printInfo).then(data => {
        this.$store.state.bluetooth.apiData.wazeFull = data
      }, error => {
        console.log(error)
      })
      ...
      google.maps.event.addListener(path, "click", event => {
        if (this.$store.state.bluetooth.modes.addFromMap) {
          if (path.strokeColor != "purple") {
            ...
            this.multigraphSegs = this.multigraphSegs.filter(s => s.info.description != data.info.description)
          }
        } else {
    ...
    getDeviceURL(level) {
      if (level == 1)
        return require('@/assets/flow-marker-1.png')
      else if (level == 2)
        return require('@/assets/flow-marker-2.png')
      else if (level == 3)
        return require('@/assets/flow-marker-3.png')
      else if (level == 4)
        return require('@/assets/flow-marker-4.png')
      else if (level == 5)
        return require('@/assets/flow-marker-5.png')
      else if (level == 6)
        return require('@/assets/flow-marker-6.png')
      else
        return require('@/assets/flow-marker-0.png')
    },
    /* Waze Data Methods */
    createWazeAlerts(dur = null, timeEnd = null) {
      if (!dur || !timeEnd) {
        ...
        this.$store.state.bluetooth.apiData.wazeFull.filter(w => (w.alertTimeTS > startTS && w.alertTimeTS < endTS)).forEach(w => {
          this.$store.state.bluetooth.subtoggles.forEach(st => {
            if (w.alertType.name == st) {
              this.createWazeAlert(w, this.$store.state.bluetooth.map);
              return
    ...
    createWazeAlert(data, map) {
      let title = data.subType && data.subType.name ? data.subType.name : 
                  (data.description ? data.description : data.alertType.name)
    ...
    addWazeClusters() {
      if (this.filteredWazeMarkers) {
        this.$store.state.bluetooth.wazeClusters = new MarkerClusterer(this.$store.state.bluetooth.map, this.filteredWazeMarkers, {
    ...
    plotShortestPath(payload) {
      ...
      this.traveltimePopup = this.createPopup(
        `Estimate travel time: ${payload.travelTime} hours`
      );
      this.detourPoly.setMap(this.$store.state.bluetooth.map);
      let recenter =
        payload.coords[Math.round((payload.coords.length - 1) / 2)];
    ...
    '$store.state.bluetooth.modes.addFromMap': {
      handler: function (newVal, oldVal) {
        if (newVal && !oldVal) {
          ...
          console.log(this.$store.state.bluetooth.segmentPolylines);

    ...
    <v-col>
      <v-row>
        <v-col cols="12" offset-lg="3" lg="6">
          <v-autocomplete v-model="selectedDevice" dense :items="deviceInfo" :loading="isLoading" :search-input.sync="search"
            color="white" hide-no-data hide-selected item-text="fullName" item-value="API" :label="label" return-object 
        ...
        <v-col cols="12" offset-lg="2" lg="8">
          <v-card class="mb-6" max-width="100%" v-if="selectedDevice">
            <v-card-actions class="px-5 py-0 pb-0">
                <DateTimePicker v-model="selectedDatetime" :dialogWidth="420" timeFormat="HH:mm:ss" v-on:update="update"
                :textFieldProps="{outlined: false, dense: true, class:'mt-4'}" v-on:updateCurr="update"
                :datePickerProps="{min: dateMin, max: dateMax, color: 'deep-purple lighten-1', headerColor: 'transparent'}"
                :timePickerProps="{min: timeMin, max: timeMax, color: 'deep-purple lighten-1', headerColor: 'transparent'}" />
              <v-spacer />
              <v-tooltip top>
                <template v-slot:activator="{ on, attrs }">
                  <v-btn small color="purple" outlined fab v-bind="attrs" v-on="on" v-if="bpInfo" class="mr-2"
...
export default {
  ...
  methods: {
    ...
    fetchBPInfo(bp) {
      if (this.submittedDateTime && bp) {
        ...
        let startStr = `${s.year}${Utils.formatXX(s.month)}${Utils.formatXX(s.day)}${Utils.formatXX(s.hour)}${Utils.formatXX(s.minute)}00`
        let endStr = `${e.year}${Utils.formatXX(e.month)}${Utils.formatXX(e.day)}${Utils.formatXX(e.hour)}${Utils.formatXX(e.minute)}00`
    ...
    fetchBP(apiReqName, start, end) {
      Api.fetchBP(apiReqName, start, end).then(data => {
        this.bpInfo = data;
      }, error => {
        console.log(error);
      })
  ...
  computed: {
    ...
    submittedDateTime() {
      ...
      if (date) {
        return DateTime.fromObject({year: date.getFullYear(), month: date.getMonth() + 1, day: date.getDate(), 
                  hour: date.getHours(), minute: date.getMinutes(), zone: 'America/New_York'})
      } else {
        return null
      }
    },
    ...
    timeMin() {
      if (this.submittedDateTime && this.startDT && 
    ...
    timeMax() {
      if (this.submittedDateTime && this.endDT && 

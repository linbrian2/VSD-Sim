  ...
  <v-app-bar app dark flat dense clipped-left color="black">
        ...
        <template v-if="inHistoricalMode">
          <v-menu v-model="menu" :close-on-content-click="false" offset-y>
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      ...
                      :disabled="
                        !fetchHistDone || $store.state.bluetooth.modes.addFromMap
                      "
                    >
            ...
            <v-card class="pa-3" width="355px">
              <v-card-actions>
                Exit out of historical mode and use current time (Realtime
                data).
              </v-card-actions>
              <v-card-actions>
                <v-spacer></v-spacer>
    ...
    <div class="button-group" v-show="$vuetify.breakpoint.mdAndUp">
      <v-btn-toggle
        ...
        dark
        v-model="$store.state.bluetooth.selectedPage"
      >
        <v-tooltip left>
          <template v-slot:activator="{ on, attrs }">
...
export default {
      ...
      switch (i) {
        ...
        case 4:
          if (this.showProgressCircle) {
            this.$store.commit('bluetooth/SET_NOTIFICATION', { show: true, text: 'Progress circle disabled.', timeout: 2500, color: 'info' } )
            this.showProgressCircle = false
          } else {
            this.$store.commit('bluetooth/SET_NOTIFICATION', { show: true, text: 'Progress circle enabled.', timeout: 2500, color: 'info' } )
  ...
  computed: {
    fetchHistDone() {
      if (this.inHistoricalMode && !!this.$store.state.bluetooth.apiData.segments && !!this.$store.state.bluetooth.apiData.waze) {
        return true
      }
      else {
        return false

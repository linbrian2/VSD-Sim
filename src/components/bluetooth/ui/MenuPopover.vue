        ...
        <v-list>
          ...
          <v-list-item>
            <v-tooltip bottom v-for="s in severity" :key="s.id">
              <template v-slot:activator="{ on, attrs }">
                <div v-bind="attrs" v-on="on" >
                <v-checkbox dense :label="s.name" v-model="s.flag" value="value" class="pr-5">
                </v-checkbox>
                </div>
              </template>
  ...
  methods: {
    init() {
      ...
      this.segments.forEach(s => {
        if (s.travelTime.level == 1 || s.travelTime.level == 2)
          this.severity[0].count++
        else if (s.travelTime.level == 3 || s.travelTime.level == 4)
          this.severity[1].count++
        else if (s.travelTime.level == 5)
          this.severity[2].count++
        else if (s.travelTime.level == 6)
          this.severity[3].count++
      })
    },
    clear() {
      this.menu = false

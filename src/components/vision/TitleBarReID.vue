  ...
  <div class="title-name">
    <v-row wrap no-gutters>
      ...
      <v-col lg="3" sm="3" xs="12">
        ...
        <v-btn small icon v-if="!$store.state.vision.showReIDVideos" @click="$store.state.vision.showReIDVideos = true" 
               class="title-btn float-right" :disabled="!hasSnippetInfo">
          <v-icon color="white"> mdi-video-off </v-icon>
        </v-btn>
        <v-btn small icon v-if="$store.state.vision.showReIDVideos" @click="$store.state.vision.showReIDVideos = false" 
               class="title-btn float-right" :disabled="!hasSnippetInfo">
          <v-icon color="white"> mdi-video </v-icon>
        </v-btn>
        <v-btn small icon @click="$store.state.vision.dialogMetadata = true" class="title-btn float-right" :disabled="!hasSnippetInfo">

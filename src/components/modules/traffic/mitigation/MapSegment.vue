<template>
  <div>
    ...
    <MapBase @map-ready="mapMounted" @center-map="centerMapHandler" @zoom-select="zoomSelectHandler">
      ...
      <GmapPolyline :path.sync="detourPath" :options="detourOptions" :zIndex="2" />

      <GmapCustomMarker
        alignment="center"
        v-for="t in trfficLightMarkers"
        :key="`LL-${t.id}`"
        :offsetX="50"
        :offsetY="0"
        :marker="t.position"
      >
        <v-chip small outlined color="green" @click="markerClicked(t)">
          <span class="white--text">{{ t.name }}</span>
        </v-chip>
      </GmapCustomMarker>
...
export default {
  ...
  data: () => ({
    ...
    title: 'INCIDENT LOCATION AND SIGNALS',
  ...
  computed: {
    ...
    segmentItems() {
      const items = [{ text: this.title, value: null }];
    ...
    trfficLightMarkers() {
      return this.filteredMarkers.filter(item => item.id.startsWith('S'));
    },
